...
class MobProg
	...
	def verify_trigger(block, line_num, section_type, ended=false)
		...
		#p trigger[:connections] unless trigger[:connections].empty?
		trigger
	end
		...
		when :a
			...
			if items.length > 1
				...
				if m = items[1].match(/^#(\d+)$/)
					if items.length == 4
						# items[3].chop! if items[3].end_with?("~")
				...
				# Now check for easier connections to match...
					...
					if m[1] =~ /^\d+$/
						#puts "#{current_line}: #{m[1].to_i} length #{m[1].length}"
	...
	def parse_condition(line, current_line, type=nil, section_type=:progs_mob)
		...
		#p connection unless connection.nil?
		return connection
...
if ARGV[0]
	files_enumerated = []
	path = "."
	# If file to parse ends with .lst, then treat it as progs.lst containing
	# one prog file per line, ending with End or $
	if ARGV[0].match(/^[\w\.\\\/_]+\.lst$/i)
		...
		# Otherwise, this is just a single mobprog file.
	...
	files_enumerated.each do |file|
		...
		#puts file
		prog_files[file] = MobProg.new(file, ARGV[1..-1])
	end

	prog_files.each do |file, prog|
		next if prog.nil?
